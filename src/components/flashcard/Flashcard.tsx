      ...
      >
        ...
        <div
          className={cn(
            ...
            'bg-white border-2 border-blue-200 rounded-2xl shadow-lg',
        ...
        >
          <div className="text-center">
            ...
            {item.reading && !isFlipped && (
              <p className="text-xl md:text-2xl text-gray-500 mb-2" lang="ja">
                {item.reading}
              </p>
            )}
            {/* 메인 콘텐츠 (한자 또는 단어) */}
            <p className="text-6xl md:text-8xl font-bold text-gray-900" lang="ja">
              {item.content}
            </p>
          </div>
        </div>
        ...
        <div
          className={cn(
            ...
            'bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-300 rounded-2xl shadow-lg',
...
function FlashcardBack({ item }: { item: FlashcardProps['item'] }) {
  ...
  if (isKanaItem(item)) {
    return (
      <div className="text-center space-y-6">
        <div>
          <p className="text-5xl md:text-7xl font-bold text-gray-900 mb-2" lang="ja">
            {item.content}
          </p>
          {item.reading && (
            <p className="text-2xl md:text-3xl text-gray-600">{item.reading}</p>
          )}
        </div>
        <div>
  ...
  if (isKanjiItem(item)) {
    return (
      <div className="w-full space-y-4">
        <div className="text-center">
          <p className="text-6xl md:text-8xl font-bold text-gray-900 mb-2" lang="ja">
            {item.content}
          </p>
          {item.reading && (
            <p className="text-xl md:text-2xl text-gray-600 mb-2" lang="ja">
              {item.reading}
            </p>
          )}
        ...
        {(item.onyomi || item.kunyomi || item.radical) && (
          <div className="border-t border-gray-300 pt-4 space-y-2 text-sm md:text-base">
            {item.radical && (
        ...
        {item.examples && item.examples.length > 0 && (
          <div className="border-t border-gray-300 pt-4">
            <p className="font-semibold text-gray-800 mb-2">사용 예시:</p>
  ...
  if (isGrammarItem(item)) {
    return (
      <div className="w-full space-y-4">
        <div className="text-center">
          <p className="text-4xl md:text-5xl font-bold text-gray-900 mb-2" lang="ja">
            {item.pattern}
          </p>
          <p className="text-xl md:text-2xl font-semibold text-blue-700 mb-4">
            {item.meaning}
          </p>
        ...
        {item.examples && item.examples.length > 0 && (
          <div className="border-t border-gray-300 pt-4">
            <p className="font-semibold text-gray-800 mb-2">예문:</p>
            <div className="space-y-3">
              {item.examples.map((ex, idx) => (
                <div key={idx} className="text-left">
                  <p lang="ja" className="text-lg md:text-xl font-medium text-gray-900">
                    {ex.sentence}
                  </p>
  ...
  return (
    <div className="text-center space-y-6">
      <div>
        <p className="text-5xl md:text-6xl font-bold text-gray-900 mb-2" lang="ja">
          {item.content}
        </p>
        {item.reading && (
          <p className="text-2xl md:text-3xl text-gray-600 mb-4" lang="ja">
            {item.reading}
          </p>
        )}
        <p className="text-3xl md:text-4xl font-semibold text-blue-700">
      ...
      {item.exampleSentence && (
        <div className="border-t border-gray-300 pt-6">
          <p className="font-semibold text-gray-800 mb-2">예문:</p>
          <p lang="ja" className="text-xl md:text-2xl font-medium text-gray-900 mb-1">
            {item.exampleSentence}
          </p>
          {item.exampleReading && (
